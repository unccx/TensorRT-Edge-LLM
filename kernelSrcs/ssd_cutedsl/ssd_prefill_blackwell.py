...
# - Adapted imports for TensorRT Edge-LLM (CuPy instead of PyTorch)
# - Added AOT export for C++ plugin integration
# - Adapted tensor wrapping for TensorRT Edge-LLM runtime
...
from ssd_prefill_blackwell_tile_scheduler import (
...
class SSDKernel:
    ...
    def _warp_tma_b_c(
        ...
        tBsB, tBgB_pre_slice = self.tma_partition_for_mma_b_operand(
            ...
            mma_tile_coord_v,
            block_in_cluster_coord_vmnk,
            tile_shape_nk=self.tile_shape_mnk_intra1[1:],
        )
...
# ============================================================================
# Test harness & AOT export (TensorRT Edge-LLM)
# ============================================================================
import cupy as cp
import numpy as np

CHUNK_SIZE = 128
AOT_N, AOT_NHEADS, AOT_DIM, AOT_DSTATE = 1, 8, 64, 128
AOT_NGROUPS, AOT_SEQLEN = 1, 128
...
def _compile_ssd_blackwell_aot(L, D, N, nheads, ngroups, batch, nchunks,
    ...
    def _cumsum_kernel(
        ...
        """Cumsum preprocessing: 1 block per (chunk, head, batch)."""
        tidx, _, _ = cute.arch.thread_idx()
        c, h, b = cute.arch.block_idx()
        t = cutlass.Int32(c) * L + tidx

        # Compute processed dt
        dtv = cutlass.Float32(0.0)
        if t < seq_len:
            dtv = cutlass.Float32(dt_in[b, t, h]) + cutlass.Float32(dt_bias[h])
            if dt_softplus:
                if dtv <= cutlass.Float32(20.0):
...
def _cumsum_numpy(dt_np, A_np, dt_bias_np, nchunks, chunk_size, dt_softplus=True):
    """NumPy cumsum preprocessing matching Triton chunk_cumsum_fwd."""
    batch, seq_len, nheads = dt_np.shape
    dt_proc = np.zeros((batch, nheads, nchunks, chunk_size), dtype=np.float32)
    dA_cumsum = np.zeros((batch, nheads, nchunks, chunk_size), dtype=np.float32)
    for b_ in range(batch):
        for h_ in range(nheads):
            for c_ in range(nchunks):
                cum = 0.0
                for l_ in range(chunk_size):
                    t_ = c_ * chunk_size + l_
                    if t_ < seq_len:
                        dtv = float(dt_np[b_, t_, h_]) + float(dt_bias_np[h_])
                        if dt_softplus and dtv <= 20.0:
                            dtv = np.log(1.0 + np.exp(dtv))
                        dt_proc[b_, h_, c_, l_] = dtv
                        cum += float(A_np[h_]) * dtv
                        dA_cumsum[b_, h_, c_, l_] = cum
    return dA_cumsum, dt_proc
...
def export_ssd_prefill_blackwell(output_dir, file_name, function_prefix, gpu_arch="",
    ...
    print(f"[ssd_prefill_blackwell] Exported to {output_dir}/{file_name}.h and .o")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Blackwell SSD prefill kernel (SM100+): test and AOT export.")
    p.add_argument("--export_only", action="store_true")
    p.add_argument("--output_dir", type=str, default="./ssd_aot_artifacts")
    p.add_argument("--file_name", type=str, default="ssd_prefill_blackwell_d64_n128")
    p.add_argument("--function_prefix", type=str, default="ssd_prefill_blackwell_d64_n128")
