...
# Ported from FlashInfer PR #2742:
#   flashinfer/gdn_kernels/blackwell_prefill/gdn_helpers.py
# Original copyright: Copyright (c) 2026 by FlashInfer team. Apache-2.0.
...
def make_smem_layout_a_kind(
    ...
    *,
    ...
    """Generate shared memory layout for tensor A.

    1. Get the partitioned shape of the A tensor based on the tiled_mma & MMA tiler.
    2. Select the heuristic SMEM layout atom based on the A tensor's majorness, the data type,
       and the major mode size.
    ...
    :type kind: cute.nvgpu.tcgen05.SmemLayoutAtomKind
...
def make_smem_layout_b_kind(
    ...
    *,
    ...
    """Generate shared memory layout for tensor B.

    1. Get the partitioned shape of the B tensor based on the tiled_mma & MMA tiler.
    2. Select the heuristic SMEM layout atom based on the B tensor's majorness, the data type,
       and the major mode size.
    3. Tile the SMEM layout atom to the MMA tile shape.
    4. Stage the SMEM layout based on the number of stages.
...
def make_smem_layout_epi_kind(
    ...
    *,
    ...
    :type epi_layout: cutlass.utils.LayoutEnum
