...
"""
Copied from https://github.com/NVlabs/alpamayo/blob/2d2b511ccd626b97aab0c9c211861c0e768551aa/src/alpamayo_r1/geometry/rotation.py
"""
...
def euler_2_so3(euler_angles: np.ndarray,
                degrees: bool = True,
    ...
    """
    return (R.from_euler(seq=seq, angles=euler_angles,
                         degrees=degrees).as_matrix().astype(np.float32))
    ...
    rot = torch.cat(
        [
            ...
            torch.tensor([0.0, 0.0, 1.0],
                         device=rot.device).repeat(rot.shape[:-2] + (1, 1)),
        ],
        dim=-2,
    )
    return rot
