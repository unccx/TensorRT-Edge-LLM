...
"""
Copied from https://github.com/NVlabs/alpamayo/blob/2d2b511ccd626b97aab0c9c211861c0e768551aa/src/alpamayo_r1/action_space/utils.py
"""
...
def construct_DTD(
    ...
    if w_smooth1 is not None:
        ...
        if isinstance(w_smooth1, float):
            w_smooth1_tensor = torch.full((*lead, max(N - 1, 0)),
                                          w_smooth1,
                                          dtype=dtype,
                                          device=device)
        else:
        ...
        DTD += lam_1 * einops.einsum(D1 * w_smooth1_tensor.unsqueeze(-1), D1,
                                     "... i j, ... i k -> ... j k")

    if w_smooth2 is not None:
        lam_2 = lam / dt**4
        if isinstance(w_smooth2, float):
            w_smooth2_tensor = torch.full((*lead, max(N - 2, 0)),
                                          w_smooth2,
                                          dtype=dtype,
                                          device=device)
        else:
        ...
        DTD += lam_2 * einops.einsum(D2 * w_smooth2_tensor.unsqueeze(-1), D2,
                                     "... i j, ... i k -> ... j k")

    if w_smooth3 is not None:
        lam_3 = lam / dt**6
        if isinstance(w_smooth3, float):
            w_smooth3_tensor = torch.full((*lead, max(N - 3, 0)),
                                          w_smooth3,
                                          dtype=dtype,
                                          device=device)
        else:
        ...
        DTD += lam_3 * einops.einsum(D3 * w_smooth3_tensor.unsqueeze(-1), D3,
                                     "... i j, ... i k -> ... j k")
...
def solve_xs_eq_y(
    ...
    *lead, N = y.shape
    ...
    while L is None:
        try:
            ridge_term = ridge * torch.eye(N, dtype=dtype,
                                           device=device).expand(*lead, N, N)
            ...
            logger.error(f"Error in cholesky decomposition: {e}",
                         exc_info=True)
            ridge *= 10
...
def dxy_theta_to_v(
    ...
    *lead, N, _ = dxy.shape
    ...
    with torch.amp.autocast(device_type="cuda", enabled=False):
        ...
        # constant.
        rhs = einops.einsum(Aw_data[..., :, 1:], b_data,
                            "... i j, ... i -> ... j")
    ...
    ridge_term = v_ridge * torch.eye(N, dtype=dtype, device=device).expand(
        *lead, N, N)
    # strip off the x_init term
