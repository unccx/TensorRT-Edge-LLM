...
"""
Adapted from https://github.com/NVlabs/alpamayo/blob/2d2b511ccd626b97aab0c9c211861c0e768551aa/src/alpamayo_r1/action_space/unicycle_accel_curvature.py to remove the inheritance from the ActionSpace base class.
"""
...
from ..geometry.rotation import (rot_2d_to_3d, rotation_matrix_torch,
                                 so3_to_yaw_torch)
from .utils import (dxy_theta_to_v, dxy_theta_to_v_without_v0, solve_xs_eq_y,
...
class UnicycleAccelCurvatureActionSpace(nn.Module):
    ...
    def __init__(
        ...
        accel_bounds: tuple[float,
                            float] = (-9.8,
                                      9.8),  # min and max bounds for accel
        curvature_bounds: tuple[float, float] = (
            -0.2, 0.2),  # min and max bounds for curvature
        dt: float = 0.1,
        n_waypoints: int = 64,
    ...
    def is_within_bounds(self, action: torch.Tensor) -> torch.Tensor:
        ...
        return torch.all(is_accel_within_bounds & is_kappa_within_bounds,
                         dim=-1)
    ...
    def estimate_t0_states(
            self, traj_history_xyz: torch.Tensor,
            traj_history_rot: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        v = dxy_theta_to_v_without_v0(dxy=dxy,
                                      theta=theta,
                                      dt=self.dt,
                                      v_lambda=self.v_lambda,
                                      v_ridge=self.v_ridge)  # (..., N+1)
        v_t0 = v[..., -1]
        return {"v": v_t0}
    ...
    def action_to_traj(
        ...
        x = (initial_x.unsqueeze(-1) + torch.cumsum(
            velocity[..., :-1] * torch.cos(theta[..., :-1]) * half_dt_term,
            dim=-1) + torch.cumsum(
                velocity[..., 1:] * torch.cos(theta[..., 1:]) * half_dt_term,
                dim=-1))  # (..., N)
        y = (initial_y.unsqueeze(-1) + torch.cumsum(
