...
/* Adapted from https://github.com/vllm-project/vllm/blob/v0.14.0/csrc/moe/marlin_moe_wna16/marlin_template.h
 * SPDX-License-Identifier: Apache-2.0
...
template <int count, trt_edgellm::marlin_dtypes::ScalarTypeId type_id>
__device__ inline void ldsm(typename MarlinScalarType<type_id>::FragA& frag_a, void const* smem_ptr)
{
...
template <trt_edgellm::marlin_dtypes::ScalarTypeId type_id>
__device__ inline void scale(
    typename MarlinScalarType<type_id>::FragB& frag_b, typename MarlinScalarType<type_id>::FragS& frag_s, int i)
{
...
template <trt_edgellm::marlin_dtypes::ScalarTypeId type_id>
__device__ inline void scale4(typename MarlinScalarType<type_id>::FragB& frag_b,
    typename MarlinScalarType<type_id>::FragS& frag_s_1, typename MarlinScalarType<type_id>::FragS& frag_s_2,
    typename MarlinScalarType<type_id>::FragS& frag_s_3, typename MarlinScalarType<type_id>::FragS& frag_s_4, int i)
{
    using scalar_t = typename MarlinScalarType<type_id>::scalar_t;
    using scalar_t2 = typename MarlinScalarType<type_id>::scalar_t2;
...
__global__ void Marlin(int4 const* __restrict__ A, // fp16 input matrix of shape mxk
    ...
    int4 const* __restrict__ b_bias_ptr,
    // float scales of input matrix, only used when is_a_8bit == true.
    // shape (m,)
    float const* __restrict__ a_scales_ptr,
    // fp16 quantization scales. shape (k/groupsize, n)
    int4 const* __restrict__ scales_ptr,
    // fp16 global scale (for nvfp4// only)
    uint16_t const* __restrict__ global_scale_ptr,
    // 4bit packed zero-points of shape
    // (k/groupsize, n/pack_factor)
    int4 const* __restrict__ zp_ptr,
    // int32 group indices of shape k
    int const* __restrict__ g_idx,
    int32_t const* __restrict__ sorted_token_ids_ptr,       // moe sorted_ids
    int32_t const* __restrict__ expert_ids_ptr,             // moe expert ids
    int32_t const* __restrict__ num_tokens_past_padded_ptr, // moe num tokens
    float const* __restrict__ topk_weights_ptr,             // moe top weights
...
)
{
    ...
    static constexpr auto a_type = trt_edgellm::marlin_dtypes::ScalarType::from_id(a_type_id);
    ...
    // Only kU8 needs explicit zero points now.
    constexpr bool has_zp = false;
    constexpr bool is_int_type = b_type == trt_edgellm::marlin_dtypes::kU4 || b_type == trt_edgellm::marlin_dtypes::kU8
        || b_type == trt_edgellm::marlin_dtypes::kS4 || b_type == trt_edgellm::marlin_dtypes::kS8
        || b_type == trt_edgellm::marlin_dtypes::kU4B8 || b_type == trt_edgellm::marlin_dtypes::kU8B128;
    // see comments of dequant.h for more details
    constexpr bool dequant_skip_flop = is_a_8bit || b_type == trt_edgellm::marlin_dtypes::kFE4M3fn
        || b_type == trt_edgellm::marlin_dtypes::kFE2M1f && s_type == trt_edgellm::marlin_dtypes::kFE4M3fn
        || has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value
        || has_zp && !is_zp_float && !(b_type == trt_edgellm::marlin_dtypes::kU8);
    ...
    int const scales_expert_stride
        = prob_n * prob_k / group_size / (b_type == trt_edgellm::marlin_dtypes::kFE2M1f ? 16 : 8);
    int const b_bias_expert_stride = prob_n / 8;
    ...
    c_scalar_t2* sh_block_topk_weights = reinterpret_cast<c_scalar_t2*>(sh_block_topk_weights_int4);
    ...
    auto read_moe_block_data = [&](int block_id) {
        ...
        cp_async4_pred(sh_block_sorted_ids_int4 + threadIdx.x,
            reinterpret_cast<int4 const*>(sorted_token_ids_ptr) + (block_id * moe_block_size / 4 + threadIdx.x),
...
#pragma unroll
    ...
    int s_gl_stride = prob_n / (b_type == trt_edgellm::marlin_dtypes::kFE2M1f ? 16 : 8);
    constexpr int s_sh_stride = 16 * thread_n_blocks / (b_type == trt_edgellm::marlin_dtypes::kFE2M1f ? 16 : 8);
...
#pragma unroll
    ...
    constexpr int sh_b_red_bias_size = sh_size_b_red_max > (sh_size_b_red_min + sh_bias_size)
        ? sh_size_b_red_max
...
#pragma unroll
            for (int i = 0; i < (b_sh_wr_iters * b_thread_vecs); i++)
            {
                ...
                int b_gl_idx
                    = b_gl_rd + (i % count) * threads + b_gl_stride * (i / count) * div_ceil(threads, b_sh_stride);
...
#pragma unroll
        ...
        {
            frag_b_quant[k % 2][i]
                = *reinterpret_cast<I4*>(&sh_b_stage[b_sh_stride * (k % b_sh_wr_iters) + b_sh_rd + i]);
        }
    };
...
#pragma unroll
            for (int i = 0; i < thread_m_blocks; i++)
            {
                mma<a_type_id, 32>(frag_a[k2][i], frag_b[0], (group_blocks == -1 ? frag_c : frag_c_tmp)[i][j][0]);
...
#pragma unroll
                                for (int k = 0; k < 4; k++)
                                    reinterpret_cast<FragC*>(frag_c)[(is_a_8bit ? 2 : 4) * 2 * m_block + j][k]
                                        += c_rd[k] + c_wr[k];
                            }
...
#pragma unroll
                for (int j = 0; j < 2 * (is_a_8bit ? 2 : 4); j++)
                {
                    ...
                    c_f16[j] = Cdtype::float2num(reinterpret_cast<float*>(
                        &frag_c)[(is_a_8bit ? 2 : 4) * 2 * 4 * (i / 4) + 4 * j + (i % 4) + delta]);
                }
...
#pragma unroll
                            ...
                            {
                                frag_s_half2[i]
                                    = Cdtype::num2num2(reinterpret_cast<c_scalar_t*>(&frag_s_half2[i])[idx]);
                            }
                        }
                    }
                }
...
#pragma unroll
            ...
            if (slice_iters)
            {
                ...
                b_gl_rd = B_expert_off + b_gl_stride * (threadIdx.x / b_sh_stride) + (threadIdx.x % b_sh_stride);
                ...
                else
                {
                    ...
                    {
                        s_gl_rd
                            = s_gl_stride * ((thread_k_blocks * slice_row) / group_blocks + threadIdx.x / s_sh_stride)
                            + s_sh_stride * slice_col + threadIdx.x % s_sh_stride;
                    }
                }
                start_pipes();
